...
static void video_task(void *arg)
{
    ...
    while (1) {
        ...
        if (pic) {
            ...
            // Not an error - just no new frame available at this polling instant
            // This is expected in producer-consumer models with timing jitter
            ESP_LOGD(TAG, "No frame available");
            continue;
        }
